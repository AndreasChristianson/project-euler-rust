...
use project_euler_rust::combinatorics::traverse_2d_vec::traverse_all_directions;
use project_euler_rust::strings::string_to_vec_of_vec_of_int::parse_2d_string;
...
fn largest_adjacent_product(numbers: &str, adjacent_count: usize) -> i128 {
    let mut queue = CircularQueue::with_capacity(adjacent_count);
    let grid = parse_2d_string(numbers);
    let directions = traverse_all_directions(&grid);
    let mut max_value = 0;
    for direction in directions {
        for n in direction {
            queue.push(n as i128);
            let product = queue.iter().fold(1, |accum, &queued_n| accum * queued_n);
...
#[test]
fn solution() {
    let ret = largest_adjacent_product(NUMBER_STRING, 4);
    assert_eq!(ret, 70600674);
}
